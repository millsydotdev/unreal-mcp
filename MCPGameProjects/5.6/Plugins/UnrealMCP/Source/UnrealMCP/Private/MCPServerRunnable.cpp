    ...
    while (bRunning)
    {
        ...
        if (ListenerSocket->HasPendingConnection(bPending) && bPending)
        {
            ...
            if (ClientSocket.IsValid())
            {
                ...
                int32 SocketMCPBufferSize = 65536;  // 64KB buffer
                ClientSocket->SetSendBufferSize(SocketMCPBufferSize, SocketMCPBufferSize);
                ClientSocket->SetReceiveBufferSize(SocketMCPBufferSize, SocketMCPBufferSize);
